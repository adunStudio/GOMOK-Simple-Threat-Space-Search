...
namespace AdunToridas {
	...
	private:
		...
		bool isFoul33(int col, int row) {
			bool row_p_1 = isValid(row + 1);
			bool row_p_2 = isValid(row + 2);
			bool row_m_1 = isValid(row - 1);
			bool row_m_2 = isValid(row - 2);
			bool col_p_1 = isValid(col + 1);
			bool col_p_2 = isValid(col + 2);
			bool col_m_1 = isValid(col - 1);
			bool col_m_2 = isValid(col + 2);
		...
		Stone getBasicStone(list<Event>& lists)
		{
			...
			for (int i = 0; i < event.size; ++i)
			{
				...
				{
					col = stone.col = event.thread[i].col;
					row = stone.row = event.thread[i].row;

					if (isValid(row) && isValid(col) && THREAT_BOARD[stone.row][stone.col] == BLANK)
						return stone;
				}
